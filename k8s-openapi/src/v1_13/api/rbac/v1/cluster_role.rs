...
impl ClusterRole {
    ...
    pub fn watch_cluster_role(
        ...
        let __url = format!("/apis/rbac.authorization.k8s.io/v1/watch/clusterroles/{name}?", name = name);
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
        }
        if let Some(field_selector) = field_selector {
...
impl ClusterRole {
    ...
    pub fn watch_cluster_role_list(
        ...
        let __url = format!("/apis/rbac.authorization.k8s.io/v1/watch/clusterroles?");
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
        }
        if let Some(field_selector) = field_selector {
